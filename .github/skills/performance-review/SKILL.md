...
A prioritized performance audit covering backend (SQLAlchemy, async FastAPI) and frontend (React rendering, React Query), reporting findings by severity with expected impact and metrics to measure.

> Always ask for baseline metrics if absent (p95 latency, query time, render timings). Separate measured findings from inferred risks.
...
- No request deduplication awareness (React Query handles this — verify `queryKey` consistency)
