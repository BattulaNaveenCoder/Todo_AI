...
Step-by-step Alembic migration guidance for async SQL Server (via aioodbc): safe `upgrade()` / `downgrade()` content, risk notes, and SQL Server compatibility checks.
...
| Step | Action |
|------|--------|
| 1 | Update SQLAlchemy model(s) in `app/models/` |
| 2 | Generate: `alembic revision --autogenerate -m "<description>"` |
| 3 | Review generated file in `alembic/versions/` — autogenerate is not always correct |
| 4 | Verify `upgrade()` implements all intended changes |
| 5 | Verify `downgrade()` properly reverses all changes in correct order |
| 6 | Check SQL Server compatibility (see below) |
| 7 | Apply: `alembic upgrade head` |
| 8 | Verify: `alembic current` + spot-check DB schema |
