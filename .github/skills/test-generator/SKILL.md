...
Generates complete, standards-compliant test files for both backend (pytest) and frontend (Vitest + RTL), following the AAA pattern with descriptive names, proper mocking, and full coverage of happy path + error cases.
...
- Example: `screen.getByRole('button', { name: /delete/i })`
